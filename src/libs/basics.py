...
def display_dataset(dataset, root_path: str = '../Project-Data', users=10, samples=8, size=2):
    """
    ...
    for i in range(users):
        ...
        j = 0
        while j < samples:
            img_path = f'{root_path}/train/{i:03d}_R0_{j}.bmp'
            if os.path.exists(img_path):
                img = load_image(img_path, True)
                if img is not None and isinstance(img, np.ndarray) and img.size > 0 and len(img.shape) >= 2:
    ...
    for i, user_images in enumerate(images):
        if user_images and len(user_images) > 0:

            heights = [img.shape[0] for img in user_images]
            if len(set(heights)) > 1:

                min_height = min(heights)
                user_images = [img[:min_height, :] if img.shape[0] > min_height else img for img in user_images]
            
            try:
                plt.figure(figsize=(size * len(user_images), size))
                stacked = np.hstack(user_images)
                display_image(stacked)
            except Exception as e:
                print(f'Warning: Could not display images for user {i}: {e}')
